# Usage
> [!important]
The scraper uses OTP-based mobile 2FA. After calling `/ws/login`, check your Wealthsimple mobile app or SMS for a 6-digit code and pass it to `/ws/scrape` within the session timeout window.
>
...
Because Wealthsimple requires a manual OTP, the flow is split across two endpoints:
...
2. **`POST /ws/scrape`** — Submits the OTP, scrapes activity since the given date, and returns normalized data as a CSV file.
...
Complete the 2FA and retrieve normalized transaction activity as a CSV.
...
| format       | string | no       | Export format — only `"csv"` is supported (default: `"csv"`) |
...
ws.send_2fa_text(otp)

df = ws.fetch_activity(since_date=datetime(2025, 1, 1))
normalized = normalize_financial_df(df)
normalized.to_csv("wealthsimple_activity.csv", index=False)
```
