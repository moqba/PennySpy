...
async def scrape_transactions(request: DownloadRequest):
    bank = RBCBank()
    bank.get_session_cookies(USERNAME, PASSWORD)
    try:
        transaction_file = bank.download_transactions(software=Software[request.software], account_info=AccountInfo[request.account_info], include=Include[request.include])
